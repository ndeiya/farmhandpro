...
const STATIC_ASSETS = [
    ...
    '/assets/js/app.js',
    '/manifest.json',
    // remote resources are loaded directly and should not be cached here
];
...
// Fetch event - network-first for API, cache-first for same-origin assets
self.addEventListener('fetch', (event) => {
    const { request } = event;
    const url = new URL(request.url);

    // Bypass any cross-origin requests entirely (avoid CORS issues)
